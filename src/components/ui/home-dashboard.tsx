...
function DashboardCard({
  ...
  const baseClasses = cn(
    "group relative rounded-xl border transition-colors px-4 py-5",
  ...
  return (
    <Link
      href={href}
      className={cn(baseClasses, variant === "risk" ? riskClasses : defaultClasses)}
    >
      ...
      <ArrowRight className={cn("size-5 transition-colors", variant === "risk" ? "text-rose-300/70 group-hover:text-rose-200" : "text-white/50 group-hover:text-white/80")} />
    </Link>
  );
}